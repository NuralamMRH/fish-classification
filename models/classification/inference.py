    ...
    def get_top_neighbors_from_embeddings(
        ...
        for query_emb in query_embeddings:
            ...
            if not selected_classes:
                top_idx = int(np.argmax(centroid_sims))
                fallback_label = self.centroid_labels[top_idx]
                fallback_sim = float(centroid_sims[top_idx])
                results.append({
                    fallback_label: {'index': None, 'similarity': fallback_sim if fallback_sim > 0 else 0.1, 'times': 1}
                })
                continue
            ...
            if len(selected_embeddings) == 0:
                ...
                for label, sim in centroid_scores.items():
                    score_map[label] = {'index': None, 'similarity': float(sim) if float(sim) > 0 else 0.1, 'times': 1}
                if not score_map:
                    top_idx = int(np.argmax(centroid_sims))
                    fallback_label = self.centroid_labels[top_idx]
                    fallback_sim = float(centroid_sims[top_idx])
                    score_map[fallback_label] = {'index': None, 'similarity': fallback_sim if fallback_sim > 0 else 0.1, 'times': 1}
            ...
            if _faiss_available:
                ...
                sel = selected_embeddings
                sel_norm = sel / (np.linalg.norm(sel, axis=1, keepdims=True) + 1e-10)
                q = query_embeddings if isinstance(query_embeddings, np.ndarray) else query_emb
                q = q / (np.linalg.norm(q) + 1e-10)
                sims = sel_norm @ q.reshape(-1, )
                order = np.argsort(-sims)[:min(topk_neighbors, len(sel))]
                distances = np.expand_dims(sims[order], axis=0)
                indices = np.expand_dims(order, axis=0)
